...
(
    ...
    -- key_rotation_interval is the default rotation period of the keys.
    key_rotation_interval BIGINT NOT NULL DEFAULT 0
);
...
(
    ...
    token_lifetime        BIGINT NOT NULL DEFAULT 0
);
...
-- blocky_authz_key_core is a table that contains keys of the service.
CREATE TABLE blocky_authz_key_core
(
    -- id is the primary key of the table.
    ...
    rotation_interval BIGINT                         NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX blocky_authz_project_id_idx ON blocky_authz_key_core (project_id);


-- blocky_authz_key_core_identifier is a table that contains identifiers of the key core.
-- it is a one to many relationship with the key table (1 key can have multiple identifiers).
-- An identifier could be a string
CREATE TABLE blocky_authz_key_core_identifier
(
    -- id is the primary key of the table.
    id         SERIAL PRIMARY KEY,

    -- core_id is the id of the key that the identifier belongs to.
    core_id    UUID
        REFERENCES blocky_authz_key_core (id)
            ON DELETE CASCADE,
    ...
    CONSTRAINT key_core_identifier_unique UNIQUE (identifier, core_id, project_id)
);


-- blocky_authz_key_identifier_core_id_idx is an index on the core_id column of the blocky_authz_key_core_identifier table.
CREATE INDEX blocky_authz_key_identifier_core_id_idx ON blocky_authz_key_core_identifier (core_id, project_id);


-- blocky_authz_key is a table that contains revisions of the key.
CREATE TABLE blocky_authz_key
(
    -- id is the primary key of the table.
    id         TEXT PRIMARY KEY,

    -- core_id is the id of the key core that the key is derived from.
    core_id    UUID REFERENCES blocky_authz_key_core (id)
        ON DELETE CASCADE,
    ...
    -- unique revision per key core.
    CONSTRAINT key_core_revision_unique UNIQUE (revision, core_id, project_id)
);


-- blocky_authz_key_core_id_created_at_idx is an index on the key_id column of the blocky_authz_key_table.
CREATE INDEX blocky_authz_key_key_id_created_at_idx ON blocky_authz_key (core_id, created_at DESC);

-- blocky_authz_key_revoked_at_idx is an index on the revoked_at column of the blocky_authz_key_table.
CREATE INDEX blocky_authz_key_revoked_at_idx ON blocky_authz_key (revoked_at);


-- blocky_authz_key_core_key_identifier is a table that contains identifiers of the key
-- it is a one to many relationship with the key table (1 key can have multiple identifiers).
CREATE TABLE blocky_authz_key_core_key_identifier
(
    -- id is the primary key of the table.
    id         SERIAL PRIMARY KEY,
    ...
    -- core_id is the id of the key core that the key was derived from.
    -- it is stored here for uniqueness purposes.
    core_id    UUID NOT NULL REFERENCES blocky_authz_key_core (id)
        ON DELETE CASCADE,
    ...
    -- unique key identifier per key.
    -- this applies also for the 'latest' key alias.
    CONSTRAINT key_identifier_unique UNIQUE (core_id, identifier)
);

-- blocky_authz_key_core_key_identifier_key_id_idx is an index on the key_id column of the blocky_authz_key_core_key_identifier table.
CREATE INDEX blocky_authz_key_core_key_identifier_key_id_idx ON blocky_authz_key_core_key_identifier (key_id);
...
-- blocky_authz_client_type is an enumeration of supported client types.
CREATE TYPE blocky_authz_client_type AS ENUM ('PUBLIC', 'CONFIDENTIAL');
...
(
    ...
    type                  blocky_authz_client_type NOT NULL,
    ...
    -- organization_internal is a flag that indicates if the client is an internal client.
    -- internal clients are clients that are used by the service itself.
    organization_internal BOOLEAN                  NOT NULL DEFAULT FALSE
);
...
(
    ...
    -- it is used to authenticate the client when it requests a token.
    secret_hash      BYTEA                    NOT NULL,

    -- created_at is the time that the credentials were created.
    created_at       TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),

    -- updated_at is the time that the credentials were last updated.
    updated_at       TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
);
...
-- blocky_authz_client_resource_permissions is a table that contains resource permissions of the client.
-- it is a many to many relationship with the client and resource permission tables.
CREATE TABLE "blocky_authz_client_resource_permissions"
(
    -- id is the primary key of the table.
    id                     SERIAL PRIMARY KEY,

    -- client_id is the id of the client that the resource permission belongs to.
    client_id              TEXT
        REFERENCES blocky_authz_client (id)
            ON DELETE CASCADE,

    -- resource_permission_id is the id of the resource permission that the client belongs to.
    resource_permission_id UUID
        REFERENCES blocky_authz_resource_permission (id)
            ON DELETE CASCADE,

    -- project_id is the id of the project that the client and resources belongs to.
    project_id             TEXT NOT NULL,

    -- unique client and resource permission per project.
    CONSTRAINT client_resource_permission_unique UNIQUE (client_id, resource_permission_id, project_id)
);

-- blocky_authz_client_resource_permissions_client_id_idx is an index on the client_id column of the blocky_authz_client_resource_permissions table.
CREATE INDEX blocky_authz_client_resource_permissions_client_id_idx ON blocky_authz_client_resource_permissions (client_id, project_id);

-- blocky_authz_client_resource_permissions_permission_id_idx is an index on the resource_permission_id column of the blocky_authz_client_resource_permissions table.
CREATE INDEX blocky_authz_client_resource_permissions_permission_id_idx ON blocky_authz_client_resource_permissions (resource_permission_id, project_id);
